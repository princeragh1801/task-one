...
const PracticeTestApp: React.FC = () => {
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white min-h-screen">
      ...
      {/* Tabs */}
      <div className="flex bg-white justify-between border-b border-gray-200 mb-4">
      <button
        ...
        <button
          className={`py-2 px-4 font-medium text-sm ${activeTab === 'Worksheet' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-500'}`}
  ...
  <button className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium transition-colors">
    Start New Practice Test
      ...
      <div className="space-y-3">
        ...
        {/* {activeTab === 'Class test' && previousTests.map(renderTestItem)}
        {activeTab === 'Worksheet' && upcomingTests.map(renderTestItem)} */}
      </div>
